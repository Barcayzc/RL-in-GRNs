...
def evaluate(model_path: Path,
    ...
    #     # --- 1) take (possibly forced) action ---------------------------------
    #     if t == 0:
    #         action = 7                       # force a real flip only once
    #     else:
    #         q_values = q_net(state[None], training=False)
    #         action   = int(tf.argmax(q_values, axis=-1)[0])

    #     next_state, r, term, trunc, _ = env.step(action)
    #     print(f"t={t:02d}, a={action:3d}, r={r:+.1f}")

    #     R     += r
    #     state  = next_state                 # keep state in sync

    #     if term or trunc:
    #         break

    # print(f"episode return = {R:+.1f}")
    ...
    #             # don’t break—still collect SSD for the rest of the 5000 steps
    ...
    for ep in tqdm(range(n_episodes), desc="DDQN-PER"):
        ...
        returns.append(ep_ret)
        if term:                              # only count *successful* completes
            success += 1
            interventions.append(flips)
